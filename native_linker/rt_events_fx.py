...
def _techset(c):
    ...
    THE 4-ALIGN (added 2026-08-08, rt-fix workflow).  Load_MaterialTechniqueSetPtr
    (0x021b84b8 guest) reads, at +0x50:

        +0x02c  bl DB_PushStreamPos(0)
        +0x038  cmpwi r31,0 / -1 / -2      (0 -> nothing; other -> ALIAS, no bytes)
        +0x050  li  r3,3
        +0x054  bl DB_AllocStreamPos       <-- mask 3 == ALIGN 4   (RPL-read, and
        +0x058  stw r3,0(r9)                   _rpl_align_table.json agrees:
        +0x08c  bl Load_Stream(1, ts, 0x88)    site 0x54 raw 3 align 4)
        +0x09c  bl DB_PushStreamPos(5)

    PROVENANCE HONESTY: the align site sits between Push(0) and Push(5), i.e. on
    the cursor the 136-byte root is streamed into, so the *mechanism* by which it
    also moves the block-5 cursor is NOT established here -- do not repeat a
    mechanism claim from this comment.  What IS established is the LINKER's
    encoding rule, forward-scored against genuine console output:

      genuine zm_transit non-entry Material.techniqueSet aliases, 3 distinct
      payloads over 18 consumers -- `0xA0000000 + align4(rt(inline ts body)) + 1`
      reproduces 18/18 exactly; without the align, 0/18.  Pads observed {1,2,3}.
        0x007A742C x5  <- rt(0x00758A45)=0x007A7429  mc_lit_sm_r0c0n0s0_7j414q0z
        0x00AA8F6C x8  <- rt(0x00A54A78)=0x00AA8F6B  mc_sw4_3d_optic_lens_4w47028j
        0x00ACE0E0 x5  <- rt(0x00A77A7E)=0x00ACE0DE  ,mc_sw4_3d_reticle_dynamic_*
      (a 19th _material_scan candidate is the known float phantom: an 'A'-named
      0x41200000 body inside XModel WeaponCamo float data -- not an alias.)

    REGRESSION BAR, measured before baking: raid_oracle_control.py output
    BYTE-IDENTICAL to _rtfix_BASELINE_raid_oracle.log; ST-dedup unchanged
    (transit 2821/0, raid 7015/0, dockside 9778/0); transit blockSize[5]
    unchanged at 116,347,512 -- the 1-3 byte pad is re-absorbed by the following
    align-4 technique allocation, so nothing downstream moves.
    Rejected alternative: root as a block-5 seg(136,4) also scores 18 but shifts
    block5 by +8192 and drifts 6 of 10 bodies -- it matched by requantisation
    luck.  Ev.skip() guards size>0, so the zero-size align event is appended
    directly; replay_events' skip branch aligns, then adds `size` (0) bytes.
    """
    import rt_events_mts as RM
    b = c.o
    c.events.append(('skip', 0, 4))            # DB_AllocStreamPos(3) -- align 4
