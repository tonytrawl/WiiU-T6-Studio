...
RULES = {
    ...
    'mat_root':  ('t', 0),
    'mat_name':  ('v', 1),
    'techset':   ('v', 1),     # inline techset asset (rare): coarse packed
    'textable':  ('v', 4),
    'img_root':  ('t', 0),
    'img_name':  ('v', 1),
...
def _material_events(c):
    ...
    if tsp in PTRS:                             # inline techset asset (rare;
        import sys, os                          # coarse: virtual packed)
        sys.path.insert(0, os.path.join(os.path.dirname(
            os.path.abspath(__file__)), '..', 'wiiu_ref'))
        import shader_probe
        end, _ = shader_probe.parse_techset(d, c.o)
        _emit(c, end - c.o, 'techset')
    if ttp in PTRS:
        defs = c.o
