...
def main():
    ...
    # THE FIX: bake the RUNTIME address, not the file VA. B9 used the raw file
    # VA and landed 0x2000 short of GetRawFile.
    target = R.rt_code(GETRAW)
    hi, lo = (target >> 16) & 0xFFFF, target & 0xFFFF
    code = [0x3D800000 | hi,     # lis   r12, hi(rt_code(GETRAW))
            0x618C0000 | lo,     # ori   r12, r12, lo
            0x7D8903A6,          # mtctr r12
            0x4E800420]          # bctr   -- ABSOLUTE indirect jump
    ...
    out = r.build()
    open(os.path.join(OUTDIR, "B11_t6mp_cafef_rpl.rpl"), "wb").write(out)
