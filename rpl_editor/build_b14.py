#!/usr/bin/env python3
"""B14 -- B13 with the .bss counter gate REMOVED. Logs unconditionally.
...
def main():
    ...
    def emit(base):
        # NO GATE. B13 was silent and did NOT crash, which indicts the gate
        # (a non-zero .bss read making `bge` always skip) rather than the call.
    ...
    out = r.build()
    open(os.path.join(OUTDIR, "B14_t6mp_cafef_rpl.rpl"), "wb").write(out)
    ...
    print(f"\n  B14: {len(out)} bytes  md5={hashlib.md5(out).hexdigest()}")
    print(f"       expect MANY '[OSConsole] [RPLEDIT] probe alive' lines -- BOOT TO MENU ONLY")
